$(document).ready(function() {
    ...
    $('#filter-works a').click(function(e) {
        ...
        for (var i = 0; i < filters.length; i++) {
            if ($(this).data(filters[i].name)) {
                filters[i].val = $(this).data(filters[i].name);
            }
        }
    ...
    function handleTweets(tweets) {
        ...
        $('.twitter_reply_icon').data({
            'toggle': 'tooltip',
            'placement': 'bottom'
        }).attr({
            'title': 'Reply'
        }).tooltip();
        $('.twitter_retweet_icon').data({
            'toggle': 'tooltip',
            'placement': 'bottom'
        }).attr({
            'title': 'Retweet'
        }).tooltip();
        $('.twitter_fav_icon').data({
            'toggle': 'tooltip',
            'placement': 'bottom'
        }).attr({
            'title': 'Favorite'
